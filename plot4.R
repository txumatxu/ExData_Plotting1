...
# Function plot4
#
# This function generates the fourth of the plots required by the first project
# of the exploratory data analysis course. It plots 4 plots in a 2 x 2 array:
#       1- Evolution of the Active Power
#       2- Evolution of the Voltage
#       3- Evolution of three Submeterings
#       4- Evolution of the Reactive Power
#
...
#
# This function returns: It creates a file called plot4.png in the working directory.
#    
#
...
plot4<- function() {
        ...
        # A 2 x 2 graphic is initialized using the appropiate mfrow value on par
        par(mfrow = c(2,2))
        
        # the first plot area is initialized
        # Global active power is plotted
        plot(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Global_active_power,ylab = 'Global Active Power',xlab="",type="n")
        lines(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Global_active_power)
        
        # the second plot area is initialized
        # The voltage evolution on time is plotted
        plot(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Voltage,ylab = 'Voltage',xlab="datetime",type="n")
        lines(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Voltage)
        
        # the third plot area is initialized
        # It contains a line showing the evolution of each of the sub meterings
        ...
        legend("topright",lwd=1, lty=c(1,1,1), col = c("black","red","blue"),bty = "n",legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
        
        # the fourth plot area is initialized
        # It plots the evolution of the Reactive Power
        plot(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Global_reactive_power,ylab = 'Global_reactive_power',xlab="datetime",type="n")
        lines(strptime(usedData$DateTime,"%d/%m/%Y %H:%M:%S"),usedData$Global_reactive_power)
